...
from Control.maintenanceLogic import getMaintenanceStatus, getMaintenanceWaitingDelevary, getMaintenanceWaitLaborCost
...
class CreateMainWaitLaborCostReport(object):
	def __init__(self):
		"""Constructor"""
	...
	def createDocument(self, canvas, doc):
		...
		ptext = "<font size=10><u><b>Maintenance's Waiting Labor Cost </b></u></font>"
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
	...
	def create_pdf(self):
		...
		for idx, val in enumerate(getMaintenanceWaitLaborCost()):
				line_data = [str(line_num), val.m_code, val.customers.name,
							 val.product_of_maintenance,val.created_at,
							 val.cost_of_bill_of_material]
		...
		for idx, val in enumerate(getMaintenanceWaitLaborCost()):
				simplelistFN.append(val.cost_of_bill_of_material)
