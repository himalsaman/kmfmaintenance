...
class CreateINVAllFPReport(object):
	def __init__(self):
		"""Constructor"""
		self.width, self.height = A4
		self.styles = getSampleStyleSheet()

		pdfname = 'invAllFP.pdf'
	...
	def create_pdf(self):
		...
		text_data = ["#", "FP. Sys. Code",  "FP. Name","FP. QTY.", "FB. Source"]
		d = []
		font_size = 8
		centered = ParagraphStyle(name="centered", alignment=TA_CENTER)
		...
		for val in select_all_finish_product():
			...
			line_data = [str(line_num), val.gen_code, val.name, val.inv_qty, val.source]
		...
		table = Table(data, colWidths=[50, 80, 180,  50, 70], rowHeights=20
