...
class CreateINVAllSPReport(object):
	def __init__(self):
		"""Constructor"""
		self.width, self.height = A4
		self.styles = getSampleStyleSheet()

		pdfname = 'invAllSP.pdf'
	...
	def create_pdf(self):
		...
		text_data = ["#", "SP. Sys. Code", "SP. Code", "SP. Name",  "RM. INV. QTY."]
		d = []
		font_size = 8
		centered = ParagraphStyle(name="centered", alignment=TA_CENTER)
		...
		for val in select_all_spare_parts():
			...
			line_data = [str(line_num), val.gen_code, val.code, val.name,
						  val.inv_qty]
		...
		table = Table(data, colWidths=[50, 80, 80, 180, 70], rowHeights=20
...
if __name__ == "__main__":
	CreateINVAllSPReport().create_pdf()
