...
class CreateDelvRecReport(object):
	def __init__(self, maint):
		...
		pdfname = 'delvrec-{}'.format(self.maint.m_code) + '.pdf'
	...
	def createDocument(self, canvas, doc):
		...
		ptext = '<font size=10><u><b>Maintenance Delivery : {}</b></u></font>'.format(
			self.maint.m_code) + '<font size=10><u><b> / {}</b></u></font>'.format(self.maint.created_at)
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
		p.drawOn(self.c, *self.coord(12, 30, mm))
		##########################################################################################
		ptext = '<font size=9><a>Created at: {}</a></font>'.format(
			self.maint.created_at)
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
		p.drawOn(self.c, *self.coord(155, 30, mm))

		ptext = '<font size=9><a>Start at: {}</a></font>'.format(
			self.maint.start_date)
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
		p.drawOn(self.c, *self.coord(155, 35, mm))

		ptext = '<font size=9><a>Finish at: {}</a></font>'.format(
			self.maint.done_date)
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
		p.drawOn(self.c, *self.coord(155, 40, mm))

		ptext = '<font size=9><a>Delivery at: {}</a></font>'.format(
			self.maint.close_at)
		p = Paragraph(ptext, style=normal)
		p.wrapOn(self.c, self.width, self.height)
		p.drawOn(self.c, *self.coord(155, 45, mm))
		##########################################################################################
	...
	def create_pdf(self):
		...
		nottxt = "<font size=11><p>I'm <u>{}</u> admitted that, I received product his data " \
				 ...
				 "described above were fixed .</p></font>".format(self.maint.customers.name)
		pnottxt = Paragraph(nottxt, styles['Normal'])
		data = [[pnottxt]]
		t = Table(data, colWidths=[500])
		story.append(t)
		# story.append(pnottxt)
		for x in range(2):
			story.append(spacer)
		#########################################################################################
		...
		cutxtnum = '<font size=11><p><u>Customer</u><br/>{}</p></font>'.format(self.maint.customers.name)
		pcutxtnum = Paragraph(cutxtnum, centered)
